...
import { ArrowLeft, Code, Briefcase, TrendingUp, Palette, ChevronRight, CheckCircle, AlertTriangle, XCircle } from "lucide-react";
...
export default function DemoReport() {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        const response = await fetch(`http://localhost:8000/api/demo/report/${reportId}`);
        ...
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            {[1, 2, 3, 4].map((num) => (
              <button
                key={num}
                onClick={() => router.push(`/demo/report/${num}`)}
