...
export default function ProjectComparison() {
  ...
  const projectIds = params.projectIds as string;

  useEffect(() => {
    const fetchComparison = async () => {
      try {
        const response = await fetch(`http://localhost:8000/api/demo/portfolio/compare/${projectIds}`);
  ...
  const ComparisonMetric = ({ label, values, unit = "", higherBetter = true }: { 
    ...
    const sortedEntries = higherBetter 
      ? entries.sort(([,a], [,b]) => {
          const aVal = typeof a === 'string' ? parseFloat(a.replace(/[^0-9.]/g, '')) : a;
          const bVal = typeof b === 'string' ? parseFloat(b.replace(/[^0-9.]/g, '')) : b;
          return bVal - aVal;
        })
      : entries.sort(([,a], [,b]) => {
          const aVal = typeof a === 'string' ? parseFloat(a.replace(/[^0-9.]/g, '')) : a;
